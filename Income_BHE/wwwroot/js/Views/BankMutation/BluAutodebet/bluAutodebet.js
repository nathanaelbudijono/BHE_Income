const BluAutodebetManager = {
    ...
    async getInsight() {
        const response = await getFetcher(`${API_URL}/bank-mutation/insight/BLUAUTODEBET`)
    ...
    setUpEventListner() {
        document.addEventListener('click', (e) => {
            if (e.target.closest('#start-tour-btn')) {
                runShepherdTour(bankMutationBluTour);
            }
        });
    },
    async init() {
...
const initBluFilesGrid = () => {
    BluAutodebetManager.state.filesGrid = $("#blu-files-grid").dxDataGrid({
        showBorders: true,
                        ...
                        .dxDropDownButton({
                            ...
                            onItemClick: function (e) {
                                switch (e.itemData.id) {
                                    case "detail":
                                        window.location.href = `${BASE_URL}/BankMutation/BluAutodebet/Detail?file=${encodeURIComponent(options.data.MutationFileName)}&bankId=${encodeURIComponent(options.data.BankId)}`;
                                        break;
                                }
                            },
                            items: [
        ...
        dataSource: getBluDataSource(),
        keyExpr: "MutationFileName"
    }).dxDataGrid("instance");
};

const getBluDataSource = (type) => {
    return new DevExpress.data.CustomStore({
        key: "MutationFileName",
        load: async function (loadOptions) {
            const dateRange = $("#bluDateRange").dxDateRangeBox("instance").option("value") || [];
...
const onBluValueChanged = (e) => {
    if (BluAutodebetManager.state.filesGrid) {
        BluAutodebetManager.state.filesGrid.refresh();
    }
};
