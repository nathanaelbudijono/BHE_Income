const VaDetailManager = {
    ...
    render() {
        this.state.dom.title.textContent = `Virtual Account Mutation - ${this.state.data.fileName}`
...
const initVirtualAccountDetailMutationGrid = () => {
    VaDetailManager.state.grid = $("#va-detail-mutation-grid").dxDataGrid({
        showBorders: true,
        ...
        dataSource: getVaDetailDataSource(),
        keyExpr: "RefNo"
    }).dxDataGrid("instance");
};

const getVaDetailDataSource = () => {
        ...
        load: async function (loadOptions) {
            ...
            try {
                ...
                if (response != null && response.data.length > 0) {
                    VaDetailManager.state.data.companyId = response?.data[0]?.CompanyId;
                    VaDetailManager.state.data.branch = response?.data[0]?.Branch;
                    VaDetailManager.state.data.retensionNumber = response?.data[0]?.RetensiNo;
                    VaDetailManager.state.data.reportNumber = response?.data[0]?.ReportNo;
                    VaDetailManager.state.data.fileName = response?.data[0]?.MutationFileName;
                    VaDetailManager.state.data.mutationDate = response?.data[0]?.MutationDate;
                    VaDetailManager.state.data.companyName = response?.data[0]?.CompanyName;
                    VaDetailManager.state.data.companyId = response?.data[0]?.CompanyId;
                    VaDetailManager.state.data.uploadDate = response?.data[0]?.EntryDate;
                    VaDetailManager.state.data.bankId = response?.data[0]?.BankId;
                    VaDetailManager.render();
                }
                return response;
