const OfflineDetailManager = {
    ...
    render() {
        this.state.dom.title.textContent = `Offline Autodebet Mutation - ${this.state.data.fileName}`
...
const initOfflineDetailMutationGrid = () => {
    OfflineDetailManager.state.grid = $("#offline-detail-mutation-grid").dxDataGrid({
        showBorders: true,
        ...
        columns: [
            ...
            { dataField: "CustomerNumber", caption: "Customer Number" },
            { dataField: "TrxNo", caption: "Transaction Number" },
            { dataField: "TrxDescription", caption: "Transaction Description" },
            { dataField: "TrxStatus", caption: "Transaction Status" },
            {
                dataField: "TrxAmount",
                ...
                cellTemplate: function (container, options) {
                    if (options.value != null) {
                        ...
                        $("<span>").text(formattedValue).appendTo(container);
                    }
                }
            }
        ],
        ...
        dataSource: getOfflineDetailDataSource(),
        keyExpr: "AccountNumber"
    }).dxDataGrid("instance");
};

const getOfflineDetailDataSource = () => {
    ...
    return new DevExpress.data.CustomStore({
        ...
        load: async function (loadOptions) {
            ...
            const payload = {
                ...
                MutationType: "offlineAutodebit",
                BankId: "BCA"
            };
            try {
                const response = await postFetcher(`${API_URL}/bank-mutation/get-by-fileName`, payload)
                if (response != null && response.data.length > 0) {
                    OfflineDetailManager.state.data.companyId = response?.data[0]?.CompanyId;
                    OfflineDetailManager.state.data.branch = response?.data[0]?.Branch;
                    OfflineDetailManager.state.data.retensionNumber = response?.data[0]?.RetensiNo;
                    OfflineDetailManager.state.data.reportNumber = response?.data[0]?.ReportNo;
                    OfflineDetailManager.state.data.fileName = response?.data[0]?.MutationFileName;
                    OfflineDetailManager.state.data.mutationDate = response?.data[0]?.MutationDate;
                    OfflineDetailManager.state.data.companyName = response?.data[0]?.CompanyName;
                    OfflineDetailManager.state.data.companyId = response?.data[0]?.CompanyId;
                    OfflineDetailManager.state.data.uploadDate = response?.data[0]?.EntryDate;
                    OfflineDetailManager.state.data.bankId = response?.data[0]?.BankId;
                    OfflineDetailManager.render();
                }
                return response;
